...
`counts` stores the gene expression value data frame of 50 genes by 520 sample, with 83 cases and 89 controls, and multiple repeated measurements (i.e. time points) per subject. Each row is a gene and each column is a sample. 
```{r, eval = TRUE, message = FALSE}
...
length(res.sol@case.ind.ref) #For cases, a list of 6 cell types' matrices. 
length(res.sol@ctrl.ind.ref) #For controls, a list of 6 cell types' matrices.
res.sol@case.ind.ref$Bcells[1:5, 1:4] #view the reference panels for B cells, for the first 5 genes and first 4 subjects.
```
...
`counts` has the gene expression value data frame of 50 genes by 520 sample, with 83 cases and 89 controls, and multiple repeated measurements (i.e. time points) per subject. Each row is a gene and each column is a sample. 
```{r, eval = TRUE, message = FALSE}
