...
dataprep_slope<-function(case_dat_se, ctrl_dat_se){
    ...
    X_sub1 = as.matrix(rbind(colData(case_dat_se)[,-(1:2)], colData(ctrl_dat_se)[,-(1:2)]))
    X_sub2 = rbind(matrix(1, nrow = N1, ncol = K), matrix(0, nrow = N2, ncol = K))*X_sub1
    X_age = c(case_age,ctrl_age)
    X_sub3 = X_sub1*X_age
    X_sub4 = rbind(matrix(1, nrow = N1, ncol = K), matrix(0, nrow = N2, ncol = K))*X_sub1*X_age  ## This is difference in slope between two groups
    X_sub4 = X_sub4
    ...
    propm = as.matrix(rbind(colData(case_dat_se)[,-(1:2)], colData(ctrl_dat_se)[,-(1:2)]))
