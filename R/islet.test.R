...
islet.test<-function(input){
  ...
  if(.Platform$OS.type == "unix") {
    ...
    mf = bplapply(X=Yall.list, islet.solve.block, datuse = input)
    #use islet.lrt.unix
    ...
    for(iTest in seq_len(input$K)){
      ...
      tmp1 = bplapply(X=Yall.list, islet.lrt.block, datuse = inputnew, ktest = iTest)
      tmp2 = unlist(tmp1)
...
#        library(BiocGenerics)})
    ...
    for(iTest in seq_len(input$K)){
      ...
      inputnew = changeinput(input, iTest)
        }else{inputnew = changeinput_slope(input, iTest)}
