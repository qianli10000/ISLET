changeinput_slope<-function(dc, iK){
    K = dc$K
    X.tmp1 = dc$X
    X.tmp2 = X.tmp1[,-(3*K+iK)] ## For slope test, the parameters being tested are: B_t[(3*K+1):(4*K)]
    dc$X = X.tmp2
    return(dc)
}
...
islet.lrt.unix_slope<-function(ipc, datuse, ktest){
...
islet.lrt.win_slope<-function(yvec, datuse, ktest){
    #exp_case = as.matrix(datuse$exp_case)
    #exp_ctrl = as.matrix(datuse$exp_ctrl)
...
islet.test_slope<-function(input){
    G = nrow(input$exp_case)
    if(.Platform$OS.type == "unix") {
        ## do some parallel computation under Unix

        mf = bplapply(seq_len(G), islet.est.bp_slope, datuse = input)
        #use islet.lrt.unix
        test.res = matrix(NA, nrow = G, ncol = input$K)
        for(iTest in seq_len(input$K)){
            inputnew = changeinput_slope(dc = input, iK = iTest)
            tmp1 = bplapply(seq_len(G), islet.lrt.unix_slope, datuse = inputnew, ktest = iTest)
            tmp2 = unlist(tmp1)
            tmp3 = unlist(lapply(mf, '[[' , 1))
        ...
        cl <- makeCluster(nworkers)
           ...
           # clusterEvalQ(cl, {
           #   require(Matrix)})
        mf = parLapply(cl, X=Yall.list, islet.est.win_slope, datuse = input)

        test.res = matrix(NA, nrow = G, ncol = input$K)

        for(iTest in seq_len(input$K)){
            inputnew = changeinput_slope(input, iTest)
            tmp1 = parLapply(cl, X=Yall.list, islet.lrt.win_slope, datuse = inputnew, ktest = iTest)
            tmp2 = unlist(tmp1)
            tmp3 = unlist(lapply(mf, '[[' , 1))
            ###obtain the p-values from each cell type
            tmp4 = LRT(tmp3, tmp2, df = 1)
